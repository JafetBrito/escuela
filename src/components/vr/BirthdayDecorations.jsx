...
// Mismo patrón de broadcast + eco local que NpcSpeechPlayer.jsx
// (LOCAL_SPEECH_EVENT) — evento hermano 'birthday_party' sobre el mismo
// canal 'vr:campus', así TODOS los conectados ven la decoración, no solo el
// cumpleañero (que es quien la dispara).
...
// Globos/confeti flotando cerca de la pantalla del Campus (CampusVideoScreen,
// [0,0,-28]) — sprites Html igual que las burbujas de diálogo, sin modelos
// 3D nuevos.
const BALLOONS = [
  { emoji: '🎈', position: [-5, 6, -26] },
  { emoji: '🎉', position: [0, 7.5, -25] },
  { emoji: '🎈', position: [5, 6, -26] },
  { emoji: '🎂', position: [-3, 4.5, -24] },
  { emoji: '🎈', position: [3, 4.5, -24] },
]
...
export default function BirthdayDecorations({ channelRef }) {
  ...
  return (
    <>
      ...
      <Html position={[0, 9.5, -22]} center distanceFactor={18}>
