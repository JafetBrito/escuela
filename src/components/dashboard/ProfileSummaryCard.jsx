    ...
    <div className="rounded-2xl border border-border bg-surface p-4">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-text-muted">{role === 'admin' ? '🛡️ Administrador' : '🎓 Estudiante'}</p>
        </div>
      ...
      <p className="mt-3 text-xs text-text-muted">
        {isMaxLevel ? '⭐ Nivel máximo alcanzado' : `${xpForNextLevel - xpIntoLevel} XP para subir de nivel`}
      </p>
      {licenseType && (
        <span className="mt-2 inline-block rounded-full border border-primary/30 bg-primary/10 px-2.5 py-0.5 text-[10px] font-bold text-primary">
          🔑 {LICENSE_LABEL[licenseType] ?? 'Sin licencia activa'}
        </span>
      )}
    </div>
  )
