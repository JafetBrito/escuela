...
// Misma tarjeta estilo "sala" que ya usa AcademiaIAPage.jsx — copiada en vez
// de importada a propósito (ver su propio comentario: es deliberadamente
// distinta de CourseCard/DashboardPage.jsx, que comparten ~20 escuelas
// genéricas que no deben cambiar de aspecto por esto).
function ChinaCourseCard({ course, pct, owned, onClick }) {
...
// Academia de China — página propia (/academia-china), separada incluso de
// la Academia de Idiomas porque cubre más que el idioma: escritura de
// caracteres y cultura. Mismo patrón de catálogo que AcademiaIAPage.jsx
// (portada + filtros + tarjetas), NO el patrón de Idiomas (ese es un
// selector genérico de 23 idiomas sin curso real detrás — ver
// LanguageSyllabusPage.jsx, todo "🔒 Próximamente").
export default function AcademiaChinaPage() {
  const navigate = useNavigate()
  const hasAccessToCourse = useAuthStore((s) => s.hasAccessToCourse)
  const progress = useProgressStore((s) => s.progress)
  const meta = CATEGORY_META.China

  const [difficulty, setDifficulty] = useState('todas')
  const [subcat, setSubcat] = useState('todas')

  const chinaCourses = useMemo(() => courses.filter((c) => c.category === 'China'), [])
  const subcategories = useMemo(
    () => Array.from(new Set(chinaCourses.map((c) => c.subcategory).filter(Boolean))),
    [chinaCourses],
  )
  const counts = useMemo(() => ({
    todas: chinaCourses.length,
    ...Object.fromEntries(DIFFICULTY_ORDER.map((d) => [d, chinaCourses.filter((c) => c.difficulty === d).length])),
  }), [chinaCourses])

  const filtered = chinaCourses.filter((c) =>
          ...
          <div className="mt-10 overflow-hidden rounded-2xl border border-border bg-surface">
            <div className="flex flex-col sm:flex-row">
              ...
              <div className="flex flex-1 flex-col gap-2 p-5">
                <p className="text-sm font-bold text-text">{meta.teacherName} · Instructor de la Academia de China</p>
                <p className="text-xs text-text-muted">
                  Pregúntale por dónde empezar, curiosidades sobre los caracteres, o cualquier duda sobre China.
                </p>
                <NpcChatPanel npcId="academia-china" npcName={meta.teacherName} npcPrompt={meta.teacherPrompt} className="mt-2" />
              </div>
            </div>
          </div>
        </div>
