...
const DIFFICULTY_STYLE = {
  principiante: { dot: 'bg-emerald-400', text: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/30' },
  intermedio: { dot: 'bg-amber-400', text: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/30' },
  avanzado: { dot: 'bg-rose-400', text: 'text-rose-400', bg: 'bg-rose-500/10', border: 'border-rose-500/30' },
}
...
function AiCourseCard({ course, pct, owned, onClick, t, lang }) {
  const style = DIFFICULTY_STYLE[course.difficulty] ?? DIFFICULTY_STYLE.principiante
  const difficultyLabel = t(`common.difficulty.${course.difficulty ?? 'principiante'}`)
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <div className={`flex items-center justify-between border-b px-4 py-2 ${style.bg} ${style.border}`}>
        <span className={`flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide ${style.text}`}>
          <span className={`h-1.5 w-1.5 rounded-full ${style.dot}`} /> {difficultyLabel}
        </span>
        {course.locked && <span className="text-[10px] font-bold text-text-muted">{t('pages.academiaIA.comingSoon')}</span>}
      </div>
      <div className="flex flex-1 flex-col gap-2 p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            {course.subcategory && <p className="text-[11px] text-text-muted">{localizeCategoryName(course.subcategory, lang)}</p>}
          </div>
        </div>
        ...
        {!course.locked && (
          <span className="mt-auto self-start rounded-lg bg-primary px-3 py-1.5 text-xs font-bold text-background transition group-hover:opacity-90">
            {owned ? (pct ? t('pages.academiaIA.continue') : t('pages.academiaIA.start')) : t('pages.academiaIA.tryFree')}
          </span>
        )}
      </div>
    </button>
...
export default function AcademiaIAPage() {
  ...
  const aiCourses = useMemo(
    () => courses.filter((c) => c.category === 'Inteligencia Artificial').map((c) => localizeCourseCatalog(c, lang)),
    [lang],
  )
      ...
      <main className="flex-1">
        ...
        <div className={`relative overflow-hidden bg-gradient-to-br ${meta.gradient} px-6 py-14 text-center sm:py-20`}>
          ...
          <div className="relative mx-auto max-w-2xl">
            ...
            <div><AcademyEnrollButton academyId="ia" /></div>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-2">
              ...
              <span className="rounded-full bg-background/20 px-3 py-1.5 text-xs font-bold text-background backdrop-blur-sm">
                {t('pages.academiaIA.pathsBadge', { count: subcategories.length })}
              </span>
              <span className="rounded-full bg-background/20 px-3 py-1.5 text-xs font-bold text-background backdrop-blur-sm">
                {t('pages.academiaIA.instructorBadge', { name: meta.teacherName })}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl px-4 py-8">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="mb-1.5 text-[10px] font-black uppercase tracking-widest text-text-muted/60">{t('pages.academiaIA.difficultyLevel')}</p>
              <div className="flex flex-wrap gap-1.5">
                ...
                >
                  {t('pages.academiaIA.all', { count: counts.todas })}
                </button>
                {DIFFICULTY_ORDER.map((d) => (
                  <button
                    ...
                    className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold transition ${
                      difficulty === d
                        ? `${DIFFICULTY_STYLE[d].bg} ${DIFFICULTY_STYLE[d].text} border ${DIFFICULTY_STYLE[d].border}`
                        : 'border border-border text-text-muted hover:text-text'
                    }`}
                  >
                    <span className={`h-1.5 w-1.5 rounded-full ${DIFFICULTY_STYLE[d].dot}`} /> {t(`common.difficulty.${d}`)} ({counts[d] ?? 0})
                  </button>
                ))}
              </div>
            </div>
            <div>
              <p className="mb-1.5 text-[10px] font-black uppercase tracking-widest text-text-muted/60">{t('pages.academiaIA.category')}</p>
              <div className="flex flex-wrap gap-1.5">
                <button
                  type="button"
                ...
                >
                  {t('pages.academiaIA.allCategories')}
                </button>
                {subcategories.map((s) => (
                  <button
                    key={s}
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {filtered.length === 0 && (
              <p className="col-span-full py-10 text-center text-sm text-text-muted">{t('pages.academiaIA.noResults')}</p>
            )}
          </div>
          ...
          <div className="mt-10 overflow-hidden rounded-2xl border border-border bg-surface">
            <div className="flex flex-col sm:flex-row">
              ...
              <div className="flex flex-1 flex-col gap-2 p-5">
                <p className="text-sm font-bold text-text">{t('pages.academiaIA.instructorTitle', { name: meta.teacherName })}</p>
                <p className="text-xs text-text-muted">
                  {t('pages.academiaIA.instructorHint')}
                </p>
