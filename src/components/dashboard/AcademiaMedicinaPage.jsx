...
function MedCourseCard({ course, pct, owned, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <div className="flex items-center justify-between border-b border-border/60 bg-emerald-500/10 px-4 py-2">
        <span className="text-[11px] font-bold uppercase tracking-wide text-emerald-400">{course.subcategory ?? course.category}</span>
...
export default function AcademiaMedicinaPage() {
  ...
  const medCourses = useMemo(() => courses.filter((c) => c.category === 'Medicina'), [])
      ...
      <main className="flex-1">
        <div className={`relative overflow-hidden bg-gradient-to-br ${meta.gradient} px-6 py-14 text-center sm:py-20`}>
          ...
          <div className="relative mx-auto max-w-2xl">
            ...
            <h1 className="mt-3 text-3xl font-black text-background drop-shadow-sm sm:text-4xl">Academia de Medicina</h1>
            <p className="mt-3 text-sm font-medium text-background/85 sm:text-base">
              Anatomía, neurociencia, patología, farmacología y el método de evaluación que usan los profesionales de
              emergencias — el conocimiento médico básico, explicado desde cero.
            </p>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-2">
              <span className="rounded-full bg-background/20 px-3 py-1.5 text-xs font-bold text-background backdrop-blur-sm">
                📚 {medCourses.length} cursos
              </span>
              <span className="rounded-full bg-background/20 px-3 py-1.5 text-xs font-bold text-background backdrop-blur-sm">
                🗂️ {subcategories.length} rutas
              </span>
              <span className="rounded-full bg-background/20 px-3 py-1.5 text-xs font-bold text-background backdrop-blur-sm">
                🦎 Lagarto Mago como instructor
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl px-4 py-8">
          <div>
            <p className="mb-1.5 text-[10px] font-black uppercase tracking-widest text-text-muted/60">Ruta</p>
            <div className="flex flex-wrap gap-1.5">
              ...
              {subcategories.map((s) => (
                ...
                >
                  {s} <span className="opacity-60">· {medCourses.filter((c) => c.subcategory === s).length}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {filtered.length === 0 && (
              <p className="col-span-full py-10 text-center text-sm text-text-muted">Ningún curso coincide con este filtro.</p>
            )}
          </div>
          ...
          <div className="mt-10 overflow-hidden rounded-2xl border border-border bg-surface">
            <div className="flex flex-col sm:flex-row">
              ...
              <div className="flex flex-1 flex-col gap-2 p-5">
                <p className="text-sm font-bold text-text">{meta.teacherName} · Instructor de Medicina</p>
                <p className="text-xs text-text-muted">
                  Pregúntale por dónde empezar, qué curso te conviene, o cualquier duda sobre los temas de la academia.
                </p>
